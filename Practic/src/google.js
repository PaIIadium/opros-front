...
const yes_no = [
  'Викладач систематично дозволяє здати на гарну оцінку завдання/лаб. роботи студентам, які погано володіють матеріалом',
  'Чи варто, на вашу думку, подовжувати контракт цьому викладачу']

const distributions = [
  'Чи задовольнила вас якість ведення практичних занять?'
]
...
function listMajors (auth, idPrepod, idPrepodsData, response) {
  ...
  asyncGet({
    ...
    for (const key of yes_no) result[key] = result[key].map(x => { return (x == 'Так') ? 100 : 0 }).reduce(avg, 0);
